...
def build_ai_prompt_template(month: str) -> str:
    return f"""你是一名专业的个人财务教练，请基于我提供的月度财务数据，输出一份结构化复盘报告。\n\n【你的任务】\n1. 用简洁语言总结本月消费结构与现金流状态。\n2. 识别值得肯定的消费习惯（至少 2 条）。\n3. 识别需要警惕的问题（至少 2 条），并解释原因。\n4. 针对下月给出可执行建议（3-5 条，需具体可落地）。\n5. 对“冲动消费比例”和“学习投资比例”给出诊断结论。\n6. 输出“消费健康度分析”：解释总分、五个维度短板和优先改进项。\n7. 输出“消费性格描述”：说明当前画像类型、形成原因和具体纠偏建议。\n8. 输出“风险解释”：基于风险雷达五维（冲动风险、订阅压力、类别集中度、消费波动度、学习投资度）说明高风险来源。\n9. 输出“报销分析”：说明本月报销金额、对现金流与预算口径的影响。\n10. 输出“净支出分析”：结合净支出（总支出-报销金额）评价预算执行的真实性。\n11. 结合订阅模块，分别从“结构成本”和“真实现金流”点评订阅压力。\n\n【数据口径说明】\n- monthly_stats: 月度收支、类别统计、标签统计、每日支出；其中 reimbursement_income 为报销收入，real_income 为真实收入（剔除报销），net_expense 为净支出（总支出-报销）\n- insights: 行为模式识别结果（异常高支出日、长期高占比类别、冲动/学习投资比例、消费健康度、消费行为画像、风险雷达）\n- budgets: 预算执行与状态（执行率按净支出口径）\n- subscriptions: 订阅口径（本月折算成本、本月实际扣费金额、本月新增/取消、下月即将扣费项目）\n\n【输出格式（严格按此结构）】\n# {month} 财务复盘\n## 1) 本月概览\n## 2) 消费结构分析\n## 3) 行为模式解读\n## 4) 消费健康度分析\n## 5) 消费性格描述\n## 6) 风险解释\n## 7) 报销与净支出分析\n## 8) 预算执行评价\n## 9) 订阅健康度\n## 10) 下月行动清单\n\n请使用中文输出，避免空泛建议，尽量引用数据中的金额、占比、趋势。"""
...
def build_ai_monthly_response(month: str) -> dict:
    ...
    monthly_stats = package.get("monthly_stats", {})
    ...
    risk_radar = package.get("risk_radar") or package.get("insights", {}).get("risk_radar", {})
    response["risk_radar"] = risk_radar
    response["risk_radar_score"] = risk_radar.get("score", 0)
